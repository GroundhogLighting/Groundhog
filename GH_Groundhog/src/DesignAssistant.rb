...
=begin  
        ...
        def self.get_radiance_materials_hash
          ...
          mats.each{|x|
            info = JSON.parse GH::Groundhog::Labeler.get_value x
            ret[x.name] = info
          }
        ...
        def self.update
          ...
          wd = GH::Groundhog.design_assistant
...
=end
        ...
        def self.get
          skp_version = Sketchup.version.split(".").shift.to_i
          
          options = {
            :dialog_title => "Design Assistant",
            :preferences_key => "GH_DAssistant",
            :scrollable => true,
            :resizable => true,
            :width => 500,
            :height => 500,
...
=begin
          ...
          wd.add_action_callback("preview") do |action_context,msg|
            ...
            if not Exporter.export(path) then
              UI.messagebox "Error while exporting... Sorry! Contact us to gmolina@GH.cl if the problem persists."
              next
            end
          ...
          wd.add_action_callback("use_luminaire") do |action_context,msg|
            ...
            m = JSON.parse(msg)
            name = m["name"]
            luminaires = definitions.select{|x| GH::Groundhog::Labeler.luminaire? x}
            definition = luminaires.select{|x| JSON.parse(GH::Groundhog::Labeler.get_value(x))["luminaire"] == name }
          ...
          wd.add_action_callback("remark") do |action_context, objective|
            GH::Groundhog.design_assistant.execute_script self.select_objective(objective)
          end
