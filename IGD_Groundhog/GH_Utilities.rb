...
class GH_Utilities
	...
	# Returns all the Sketchup::ComponentDefinition within an array.
	# @author German Molina
	# @param entities [Array<entities>] Array with entities.
	# @return [Sketchup::Faces] An array of faces.
	def self.get_component_definitions(entities)
		ret=[]
		entities.each do |k|
			if GH_Labeler.component_definition?(k) then
				ret=ret+[k]
			end
		end
		return ret
	...
	# Get the first label SketchUp::ComponentDefinition (Image, Group or Component)
	#
	# Returns all those first-level definitions that are being used.
	# @author German Molina
	...
	def self.get_first_level_definitions(definitions)
		ret=[]
		definitions.each do |ent|
			next if ent.count_instances<1 # Only used components and
			ret=ret+[ent] if ent.instances[0].parent.is_a?(Sketchup::Model)
		end
		return ret
	end
	...
	def self.hide_show_specific(a)
		...
		begin
			model=Sketchup.active_model
			model.start_operation( "Hide/Show Specific" )
		  
			faces.each do |ent| #For all faces in the model
				if GH_Labeler.is?(ent,a) then #if they are part of the class
	...
	def self.get_horizontal_faces(entities)
		ret=[]
		entities.each do |k|
			if GH_Labeler.face?(k) and self.is_horizontal?(k) then
				ret=ret+[k]
			end
		end
		return ret
